...
impl LayoutTree {
    pub fn rows(start_id: u32, window_count: u32, stacks: (u32, u32), screen: &Rect) -> LayoutTree {
        let total_stacks = (stacks.0 + stacks.1).max(1);
        build_rows(start_id, window_count, total_stacks, screen)
    }
}

fn build_rows(start_id: u32, zones: u32, stacks: u32, _screen: &Rect) -> LayoutTree {
    if zones == 0 {
        return LayoutTree::Leaf(start_id);
    }

    let stack_counts = distribute_rows(zones, stacks);
    let mut next_id = start_id;
    let mut columns = Vec::with_capacity(stack_counts.len());

    for &count in &stack_counts {
        columns.push(build_column(next_id, count));
        next_id += count;
    }
